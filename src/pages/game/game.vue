<template>
  <view class="page game-hub-page">
    <view class="hub-stage" :style="stageStyle">
      ...
      <view class="hub-grid">
        ...
        >
          ...
          <view
            class="hub-card-top"
            :class="{ 'with-cover': !!item.coverImage }"
          >
          ...
          <text v-if="!item.coverImage" class="hub-card-title">{{
            item.title
          }}</text>
        </view>
      </view>
    ...
    <view
      v-if="isCategoryPickerVisible"
      class="category-picker-mask"
      ...
      <view class="category-picker-panel" @click.stop>
        ...
        <view class="category-picker-header">
          <view class="category-picker-title-wrap">
            ...
            <text class="category-picker-subtitle"
              >每次会随机抽题，先选一个你喜欢的分类</text
            >
          </view>
          <text class="category-picker-close" @click="closeCategoryPicker"
            >关闭</text
          >
        </view>
        ...
        <view
          ...
          class="category-picker-state category-picker-state--error"
        >
          <text>{{ categoryError }}</text>
          <view class="category-picker-retry" @click="reloadCategories"
            >重新加载</view
          >
        </view>
        ...
        <scroll-view
          ...
          scroll-y
          :scroll-top="categoryPickerScrollTop"
        >
          <view
          ...
          >
            ...
            <view class="category-option-cover">
              <image
                v-if="item.image"
                class="category-option-image"
                :src="item.image"
                mode="aspectFill"
              />
              <view v-else class="category-option-fallback">{{
                item.name.slice(0, 1)
              }}</view>
            </view>
            <view class="category-option-main">
              ...
              <view class="category-option-tags">
                <text class="category-option-meta">{{
                  item.cardCountText
                }}</text>
...
const ALLOWED_TONES = new Set<DisplayGameTone>([
  "gold",
  "blue",
  "green",
  "pink",
...
const ALLOWED_LAUNCH_MODES = new Set<DisplayGameLaunchMode>([
...
function normalizeRoutePath(
  path: unknown,
  fallback = "/pages/game/coming-soon",
) {
...
function openGame(item: GameBlock) {
  if (item.launchMode === "listen-pick-category") {
    ...
    listenPickRoutePath.value = normalizeRoutePath(
      item.routePath,
      DEFAULT_LISTEN_PICK_ROUTE,
    );
    openCategoryPicker();
